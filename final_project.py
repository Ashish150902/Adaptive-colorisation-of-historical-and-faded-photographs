...
Original file is located at
    https://colab.research.google.com/drive/1O4aMmczfvypxTQ01kboejW5oVScABYKq
...
def lab2rgb(L, ab):
    ...
    for img in Lab:
        img_rgb = color.lab2rgb(img)
        img_rgb = np.clip(img_rgb, 0, 1)  # Clip values to be in the valid range
        rgb_imgs.append(img_rgb)
    return np.stack(rgb_imgs, axis=0)

def calculate_hue_accuracy(original_hsv, colorized_hsv):
    """
    Calculate the Hue Accuracy metric.
    Assume that original_hsv and colorized_hsv are numpy arrays of shape (H, W, 3).
    """
    original_hue = original_hsv[..., 0]
    colorized_hue = colorized_hsv[..., 0]
    diff = np.abs(original_hue - colorized_hue)
    accurate_pixels = np.sum(diff < 0.1)  # Adjust the threshold as needed
    total_pixels = original_hue.size
    hue_accuracy = accurate_pixels / total_pixels
    return hue_accuracy
    ...
    with torch.no_grad():
        for batch in val_dl:
            ...
            for i in range(len(Ls_val)):
                # Convert to images for score calculation
                ...
                original_hsv = color.rgb2hsv(original_color_image)
                colorized_hsv = color.rgb2hsv(colorized_image)
                ...
                accuracy = metrics.structural_similarity(original_lab, colorized_lab, channel_axis=-1)
                accuracy_values.append(accuracy)

                # Store fake color scores for later analysis
                output_scores.append((hue_accuracy, accuracy, i, original_color_image, colorized_image))

    # Calculate average scores
    avg_hue_accuracy = np.mean(hue_accuracy_values)
    avg_accuracy = np.mean(accuracy_values)

    # Print the average metrics
    print(f"Average Hue Accuracy: {avg_hue_accuracy}")
    print(f"Average Accuracy: {avg_accuracy}")

    # Sort and display top high fake average score values
    output_scores.sort(key=lambda x: (x[0] + x[1]) / 2, reverse=True)
    top_n = 5  # Number of top scores to display
    for idx in range(top_n):
        hue_acc, acc, i, original_image, output_image = output_scores[idx]
        avg_score = (hue_acc + acc) / 2
        print(f"Top {idx+1} - Avg Score: {avg_score}, Hue Accuracy: {hue_acc}, SSIM: {acc}")
...
# Assuming gan is your model and val_dl is your validation dataloader
